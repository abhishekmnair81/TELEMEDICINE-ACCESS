...
* **In-Call Chat**: Exchange text messages and guidance concurrently with the video stream.
...
| | `/api/auth/login/` | `POST` | Exchanges credentials for JWT access/refresh tokens |
...
## 🔒 Security Features
* **Role-Based Routing (RBAC)**: Route middleware validates user roles before mounting patient/doctor/pharmacist pages.
* **Token Auth (SimpleJWT)**: Secure stateless API calls using JSON Web Tokens with automated blacklist handling.
* **CSRF & CORS Safety**: Configured to restrict requests solely to allowed origins (e.g., frontend running on port 3000).
* **Local AI Execution**: Patient conversations never leave the local environment, satisfying strict HIPAA/privacy considerations.

---

## 🔄 Recent Updates & Refactoring (June 2026)

We have recently refactored the project to enhance responsiveness, performance, and stability:

* **Tailwind CSS & Responsiveness Refactoring**: Modernized Dashboard, Doctor Dashboard, Patient Dashboard, and Pharmacist Dashboard layouts to use Tailwind CSS, ensuring 100% responsiveness on mobile, tablet, and desktop screens.
* **WebRTC Video Consultation Redesign**: Upgraded the video consultation layout, fixed alignment issues under different states, and fully transitioned the UI styling to Tailwind.
* **Rural Telemedicine Branding**: Rebranded the application globally, replacing "Apollo" mentions with "Rural Telemedicine".
* **URL-Based Profile Pictures & Assets**: Replaced file-based uploads with image URL inputs across all profile setups (Doctor, Patient, Pharmacist) and medicine entries to prevent local media upload crashes and optimize memory.
* **Celery Consolidated Backend & Requirements**: Consolidated all backend dependencies into a single `requirements.txt` file (removing duplicate `requirements_celery.txt`). Enhanced configuration files to support both local development (SQLite, default Redis) and production settings (PostgreSQL, Daphne, Celery tasks, and WhiteNoise).
* **Persistent Language Selector**: Introduced a multi-language toggle integrated across dashboards with persistent storage of selected locales.
* **AI Chatbot Voice and TTS Cleanup**: Refactored `ChatInterface.jsx` to resolve conflicting merge issues with voice recognition and Text-to-Speech playback.
