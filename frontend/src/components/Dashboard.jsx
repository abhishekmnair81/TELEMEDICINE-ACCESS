  ...
  const locateAndUpdate = (onSuccess) => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        if (leafletMapInstance.current && window.L) {
          const zoom = accuracy <= 25 ? 19 : accuracy <= 100 ? 17 : accuracy <= 500 ? 15 : 13
        ...
        const accuracyNote = accuracy > 500 ? ' (approx — drag pin to refine)' : ` (±${Math.round(accuracy)}m)`
        showToast(`📍 Location detected${accuracyNote}`, 'success')
      },
      async (err) => {
      ...
      <div className="rural-primary-nav-strip bg-gray-50 border-b border-gray-100 overflow-x-auto whitespace-nowrap scrollbar-none py-1 md:py-2">
        <div className="rural-wrapper max-w-7xl mx-auto px-4 md:px-8">
          <ul className="primary-nav-links flex items-center gap-4 md:gap-6 text-xs md:text-sm font-semibold text-gray-600">
            ...
            <li className="hover:text-green-600 py-1.5 md:py-2 transition-colors duration-150"><a href="/lab-tests" className="no-underline" style={{ textDecoration: 'none' }}>Lab Tests <span className="nav-new-badge bg-green-100 text-green-800 text-[9px] px-1.5 py-0.5 rounded-full ml-1 font-bold">New</span></a></li>
            <li className="hover:text-green-600 py-1.5 md:py-2 transition-colors duration-150"><a href="#wellness-membership" className="no-underline" style={{ textDecoration: 'none' }} onClick={(e) => { e.preventDefault(); showToast("✨ Circle Wellness Membership is coming soon! Stay tuned for premium family health benefits.", "success"); }}>Circle Membership <span className="nav-new-badge bg-amber-100 text-amber-800 text-[9px] px-1.5 py-0.5 rounded-full ml-1 font-bold">Upcoming</span></a></li>
