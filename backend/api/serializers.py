...
class UserSerializer(serializers.ModelSerializer):
    ...
    profile_picture_url = serializers.CharField(required=False, allow_null=True, allow_blank=True)
    ...
    def to_representation(self, instance):
        ...
        if not data.get('profile_picture_url') and instance.profile_picture:
            request = self.context.get('request')
            if request:
                data['profile_picture_url'] = request.build_absolute_uri(instance.profile_picture.url)
            else:
...
SUPPORTED_LANGUAGES = ['English', 'Hindi', 'Kannada', 'Tamil', 'Telugu', 'Malayalam']
...
class ChatMessageSerializer(serializers.Serializer):
    ...
    elaborate = serializers.BooleanField(required=False, default=False)

    def validate_language(self, value):
        if value and value not in SUPPORTED_LANGUAGES:
            # Fall back to English for unknown languages instead of rejecting
            return 'English'
...
class PharmacyMedicineSerializer(serializers.ModelSerializer):
    ...
    def to_internal_value(self, data):
        # Ensure we have a mutable dict or QueryDict
        ...
        if 'image_urls' in data:
            val = data.get('image_urls')
            if isinstance(val, str):
                try:
                    import json
                    data['image_urls'] = json.loads(val)
                except Exception:
                    try:
                        data['image_urls'] = [x.strip() for x in val.split(',') if x.strip()]
                    except Exception:
                        pass

        # Normalize the 'form' field to match MEDICINE_FORMS choice keys or default to blank
        if 'form' in data:
            form_val = data.get('form')
            if form_val:
                valid_forms = [choice[0] for choice in Medicine.MEDICINE_FORMS]
                normalized = str(form_val).strip().lower()
                
                if normalized in valid_forms:
                    ...
                    if 'tablet' in normalized:
                        mapped = 'tablet'
                    elif 'capsule' in normalized:
                        mapped = 'capsule'
                    elif 'syrup' in normalized or 'suspension' in normalized:
                        mapped = 'syrup'
                    elif 'injection' in normalized or 'vial' in normalized or 'ampoule' in normalized:
                        mapped = 'injection'
                    elif 'cream' in normalized:
                        mapped = 'cream'
                    elif 'ointment' in normalized:
                        mapped = 'ointment'
                    elif 'drop' in normalized:
                        mapped = 'drops'
                    elif 'powder' in normalized or 'granules' in normalized or 'sachet' in normalized:
                        mapped = 'powder'
                    elif 'spray' in normalized:
                        mapped = 'spray'
                    elif 'inhaler' in normalized:
                        mapped = 'inhaler'
                    elif 'patch' in normalized:
                        mapped = 'patch'
                    elif 'gel' in normalized:
                        mapped = 'gel'
                    elif 'lotion' in normalized:
                        mapped = 'lotion'
                    
                    data['form'] = mapped if mapped else ''
    ...
    def update(self, instance, validated_data):
        ...
        if image_urls is not None:
            instance.image_url = image_urls[0] if len(image_urls) > 0 else None
            instance.images.filter(image_url__isnull=False).delete()
...
class MedicationReminderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'is_active', 'reminder_enabled', 'email_reminders_enabled',
            'meal_timing', 'notes',
