...
def send_otp_for_login(request):
    try:
        ...
        # Delete globally expired OTPs from database to keep it clean
...
def chat_stream(request):
    try:
        ...
        conversation_id = data.get('conversation_id') or request.data.get('conversation_id')
        elaborate = data.get('elaborate', False)
        is_voice = data.get('is_voice', False) or request.data.get('is_voice', False)
        from .helpers import (
        ...
        # Priority: native script > user selected > auto-detected
        ...
        if native_detected:
            ...
            logger.info(f"[chat_stream] Native script detected: {language}")
        elif user_selected_language:
            language = user_selected_language
            logger.info(f"[chat_stream] Respecting user selected: {language}")
        else:
        ...
        history = []
        if conversation:
            history = list(ChatHistory.objects.filter(
                conversation=conversation
            ).exclude(
                id=user_message.id
            ).order_by('created_at').values('role', 'message'))

        emergency_prefix = ""
        if emergency_level == 'critical':
            # Do not prepend emergency prefix; get_response returns the exact multi-language warning
            emergency_prefix = ""
        ...
        def generate():
            """
            SSE generator — emits one word-level token per data: frame.

            Raw LLM chunks (Groq/Ollama) can be multi-word strings or even full
            sentences.  Emitting them as-is means the browser receives all text
            in 1-3 huge bursts rather than a real-time word-by-word stream.

            Strategy: split every raw chunk on whitespace and re-emit each
            token as its own SSE frame.  This gives the frontend a true
            word-per-event drip that the StreamingMessage animator can render
            in real time without needing to fake-animate already-complete text.
            """
            import re as _re

            def _iter_words(text: str):
                """Split text into (word, trailing_space) pairs, preserving newlines."""
                # Split on whitespace boundaries but keep the separator so we
                # preserve newlines and multi-space formatting.
                tokens = _re.split(r'(\s+)', text)
                for tok in tokens:
                    if tok:   # skip empty strings from split
                        yield tok
            ...
            try:
                ...
                if emergency_prefix:
                    ...
                    # emit prefix word-by-word so it also animates
                    for word_tok in _iter_words(emergency_prefix):
                        yield f"data: {json.dumps({'chunk': word_tok})}\n\n"

                logger.info(f"[chat_stream]  Generating response in: {language}")

                for chunk in chatbot.get_response(msg, language, is_voice=is_voice, history=history):
                    if not chunk:
                        continue
                    full_response += chunk
                    # ── KEY CHANGE ──────────────────────────────────────────
                    # Sub-split each raw LLM chunk into individual word tokens
                    # so the frontend receives exactly one word per SSE frame.
                    for word_tok in _iter_words(chunk):
                        yield f"data: {json.dumps({'chunk': word_tok})}\n\n"
                    # ────────────────────────────────────────────────────────
                ...
                if not show_hospitals:
                    ...
                    if ai_suggests_doctor or emergency_level == 'critical':
                        show_hospitals = True
                        logger.info("[chat_stream] Hospital trigger / critical — enabling hospital finder")
                ...
                logger.info(f"[chat_stream] 📤 done | show_hospitals={show_hospitals} | lang={language}")
                logger.info("="*60)
...
def chat_view(request):
    try:
        ...
        def generate():
            try:
                for chunk in chatbot.get_response(message, language, elaborate, is_voice=is_voice):
...
def chat_with_image(request):
    ...
    try:
        logger.info("=" * 60)
        logger.info("CHAT WITH FILE REQUEST (IMAGE OR DOCUMENT)")
        logger.info("=" * 60)
        ...
        # Accept file under 'image', 'document', or 'file' keys
        uploaded_file = request.FILES.get('image') or request.FILES.get('document') or request.FILES.get('file')
 
        logger.info(f"User: {user_id} | Lang: {language} | Elaborate: {elaborate}")
        ...
        if uploaded_file.size > 10 * 1024 * 1024:
            return JsonResponse({'error': 'File must be less than 10 MB'}, status=400)
 
        content_type = uploaded_file.content_type or ''
        filename = uploaded_file.name.lower()
        
        # Check if it is a document or an image
        is_document = False
        if (content_type in ['application/pdf', 'text/plain'] or 
            'word' in content_type or 
            filename.endswith(('.pdf', '.docx', '.doc', '.txt'))):
            is_document = True

        if not is_document:
            allowed_image_types = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp']
            if content_type not in allowed_image_types and not filename.endswith(('.jpg', '.jpeg', '.png', '.webp')):
                return JsonResponse(
                    {'error': f'Unsupported file type: {content_type}. Use images (JPG, PNG, WebP) or documents (PDF, DOC, DOCX, TXT).'},
                    status=400,
                )
 
        # ── 2. Save file to media storage ──────────────────────────
        from django.core.files.storage import default_storage
        import os as _os
        import uuid as _uuid
        
        safe_name = "".join([c for c in uploaded_file.name if c.isalnum() or c in ['.', '_', '-']]).strip()
        folder = "chat_files" if is_document else "chat_images"
        saved_name = f"{folder}/{_uuid.uuid4()}_{safe_name}"
        saved_path = default_storage.save(saved_name, uploaded_file)
        
        temp_path = default_storage.path(saved_path)
        file_relative_url = default_storage.url(saved_path)
        
        logger.info(f"Saved file to media storage: {temp_path} (URL: {file_relative_url})")
            ...
            if conversation_id:
                try:
                    if request.user and request.user.is_authenticated:
                        ...
                        conversation = Conversation.objects.get(
                            id=conversation_id, user_id_anonymous=user_id
                        )
                except Conversation.DoesNotExist:
                    logger.warning(f"Conversation {conversation_id} not found, creating new one.")
 
            if not conversation:
                conv_kwargs = {'language': language}
                if request.user and request.user.is_authenticated:
                    conv_kwargs['user'] = request.user
                else:
                    conv_kwargs['user_id_anonymous'] = user_id
                conversation = Conversation.objects.create(**conv_kwargs)
                logger.info(f"Created conversation: {conversation.id}")
 
            # Save user message
            # For documents, if user_message is blank, summarize what they did
            display_user_msg = user_message
            if not display_user_msg:
                if is_document:
                    display_user_msg = f"Please analyze this document: {uploaded_file.name}"
                else:
                    display_user_msg = "Please analyze this medical image."

            ChatHistory.objects.create(
                conversation=conversation,
                user_id=user_id,
                role='user',
                message=display_user_msg,
                language=language,
                has_image=True, # We use has_image=True to signify there's a file attachment URL
                image_description=file_relative_url,
            )
 
            # Set title from first message
            if not conversation.title:
                title = display_user_msg[:50]
                if len(display_user_msg) > 50:
                    title += '...'
                conversation.title = title
                conversation.save(update_fields=['title'])
 
        except Exception as db_err:
            logger.warning(f"DB ops failed (non-fatal): {db_err}")
        ...
        def generate():
            ...
            def _iter_words(text: str):
                """Yield individual word/space tokens from text."""
            ...
            full_response = ''
            try:
                from .helpers import get_chatbot
                chatbot = get_chatbot()
 
                # We will extract text or render pages if scanned PDF
                is_scanned_pdf = False
                pdf_page_image_bytes = None
                ...
                if is_document:
                    ...
                    try:
                        if ext == 'txt' or content_type == 'text/plain':
                            ...
                            try:
                                ...
                                # Check if it is a scanned PDF (no text could be extracted)
                                if not doc_text and len(doc) > 0:
                                    logger.info(f"PDF has no extractable text, rendering scanned pages as image fallback.")
                                    try:
                                        from PIL import Image as _Image
                                        import io as _io
                                        
                                        images = []
                                        # Render up to 3 pages to handle standard reports/scans
                                        for page_num in range(min(len(doc), 3)):
                                            page = doc[page_num]
                                            pix = page.get_pixmap(dpi=150)
                                            img_data = pix.tobytes("png")
                                            images.append(_Image.open(_io.BytesIO(img_data)))
                                            
                                        if images:
                                            width = max(img.width for img in images)
                                            height = sum(img.height for img in images)
                                            combined = _Image.new("RGB", (width, height), (255, 255, 255))
                                            ...
                                            pdf_page_image_bytes = out_buf.getvalue()
                                            is_scanned_pdf = True
                                    except Exception as img_err:
                                        logger.error(f"Failed to combine pages: {img_err}, falling back to page 0")
                                        page = doc[0]
                                        pix = page.get_pixmap(dpi=150)
                                        pdf_page_image_bytes = pix.tobytes("png")
                                        is_scanned_pdf = True
                            except Exception as pdf_err:
                                logger.error(f"PyMuPDF failed: {pdf_err}, falling back to pypdf")
                                import pypdf
                            ...
                            try:
                                ...
                                # Fallback: read file printable characters
                                with open(temp_path, 'rb') as f:
                                    raw_data = f.read()
                                words = _re.findall(rb'[a-zA-Z0-9\s\.,;:!\?\-\(\)]{4,}', raw_data)
                                doc_text = "\n".join([w.decode('ascii', errors='ignore') for w in words])
                        
                        doc_text = doc_text.strip()
                    except Exception as ext_err:
                        logger.error(f"Failed to extract document text: {ext_err}")
                        doc_text = ""

                    if not doc_text and not is_scanned_pdf:
                        msg = f"Could not extract text from the document '{uploaded_file.name}'. Please make sure it is a valid PDF, Word, or Text document containing readable text."
                        yield f"data: {json.dumps({'chunk': msg})}\n\n"
                        yield f"data: {json.dumps({'done': True, 'conversation_id': str(conversation.id) if conversation else None})}\n\n"
                        return

                    if is_scanned_pdf and pdf_page_image_bytes:
                        if not chatbot.available_vision_providers:
                            msg = (
                                "No vision AI provider is configured. "
                                "Please set GROQ_API_KEY, ANTHROPIC_API_KEY, "
                                "OPENAI_API_KEY, or GEMINI_API_KEY in your .env and restart Django. "
                                "For now, please consult a healthcare professional directly."
                            )
                            yield f"data: {json.dumps({'chunk': msg})}\n\n"
                            yield f"data: {json.dumps({'done': True, 'conversation_id': str(conversation.id) if conversation else None})}\n\n"
                            return
                        ...
                        import io
                        img_buf = io.BytesIO(pdf_page_image_bytes)
                        logger.info("Starting vision analysis on scanned PDF page 0 image.")
                        for chunk in chatbot.analyze_medical_image(
                            image_buffer=img_buf,
                            user_message=user_message or 'Please analyze this scanned report.',
                            language=language,
                            elaborate=elaborate,
                        ):
                            if not chunk:
                        ...
                        # Truncate text context if it is too large (to fit in context window)
                        ...
                        history = []
                        if conversation:
                            # Exclude the latest user message we just created
                            from django.utils import timezone
                            history = list(ChatHistory.objects.filter(
                                conversation=conversation
                            ).exclude(
                                role='user', created_at__gte=timezone.now() - timezone.timedelta(seconds=1)
                            ).order_by('created_at').values('role', 'message'))

                        prompt_msg = (
                            f"The user has uploaded a document: {uploaded_file.name}\n"
                            f"Here is the text extracted from the document:\n"
                            f"\"\"\"\n{doc_text}\n\"\"\"\n\n"
                            f"Please analyze this document text and answer the user's query.\n"
                            f"User's query: {user_message or 'Please summarize and analyze this document.'}"
                        )

                        logger.info(f"Sending document prompt to chatbot. Text size: {len(doc_text)} chars")
                        for chunk in chatbot.get_response(prompt_msg, language=language, history=history):
                            if not chunk:
                                continue
                            full_response += chunk
                            for word_tok in _iter_words(chunk):
                    ...
                    # Existing Image vision analysis code
                    if not chatbot.available_vision_providers:
                        msg = (
                            "No vision AI provider is configured. "
                            "Please set GROQ_API_KEY, ANTHROPIC_API_KEY, "
                    ...
                    with open(temp_path, 'rb') as img_buf:
                        logger.info(f"Starting vision analysis. Providers: {chatbot.vision_priority_order}")
                        for chunk in chatbot.analyze_medical_image(
                            image_buffer=img_buf,
                            user_message=user_message or 'Please analyze this medical image.',
                            language=language,
                            elaborate=elaborate,
                        ):
                            if not chunk:
                ...
                logger.error(f"Analysis error: {err}", exc_info=True)
                error_text = (
                    "I encountered an error analyzing this file. "
                    "Please try again or consult a healthcare professional."
                )
                full_response = error_text
                yield f"data: {json.dumps({'chunk': error_text})}\n\n"
                ...
                try:
                    if conversation and full_response:
                        ...
                        conversation.last_message_at = _tz.now()
                        conversation.message_count   = conversation.messages.count()
                        conversation.save(update_fields=['last_message_at', 'message_count'])
                        logger.info("Assistant message saved.")
                except Exception as save_err:
                    logger.warning(f"Could not save assistant message: {save_err}")
        ...
        logger.error(f"chat_with_image critical error: {e}", exc_info=True)
...
class PharmacyMedicineViewSet(viewsets.ModelViewSet):
    ...
    def upload_images(self, request):
        try:
            ...
            medicine_data = {
                ...
                'image_urls': request.POST.get('image_urls') or request.data.get('image_urls'),
            }
            ...
            if images:
                ...
                start_order = medicine.images.count()
    ...
    def update_images(self, request, pk=None):
        try:
            ...
            medicine_data = {}
            fields_list = [
                'name', 'generic_name', 'category', 'manufacturer', 'brand_name', 'brand',
                'description', 'form', 'strength', 'price', 'mrp', 'stock_quantity',
                'requires_prescription', 'expiry_date', 'storage_instructions', 
                'batch_number', 'image_urls'
            ]
            for field in fields_list:
                val = request.POST.get(field) or request.data.get(field)
                if val is not None and val != '':
                    # Map 'brand' to 'brand_name' for backend compatibility
                    if field == 'brand':
                        medicine_data['brand_name'] = val
                    else:
                        medicine_data[field] = val
...
def text_to_speech(request):
    try:
        ...
        # Translate Romanized text to native script on-the-fly for crystal-clear TTS audio synthesis
        synthesis_text = convert_romanized_to_native(text, language)
...
class VideoConsultationRoomViewSet(viewsets.ModelViewSet):
    ...
    def cleanup_expired_rooms(self):
        try:
            ...
            expired_rooms = VideoConsultationRoom.objects.filter(
                status__in=['scheduled', 'waiting'],
                scheduled_time__lt=thirty_minutes_ago,
                started_at__isnull=True
            )
            count = expired_rooms.count()
            if count > 0:
                logger.info(f"[Room Cleanup] Removing {count} expired rooms that did not start within 30 minutes of scheduled time.")
                expired_rooms.delete()
        except Exception as e:
            logger.error(f"[Room Cleanup] Error cleaning up expired rooms: {str(e)}")

    def get_queryset(self):
        self.cleanup_expired_rooms()
        return VideoConsultationRoom.objects.all().order_by('-scheduled_time')
    ...
    def end_call(self, request):
        try:
            ...
            try:
                ...
                return Response({
                    'success': True,
                    'message': 'Consultation ended successfully (room already removed)'
                })
            ...
            room.save()
            if room.appointment:
                room.appointment.status = 'completed'
            ...
            logger.info(f"[VideoConsultationRoom] Deleting room {room.room_id} as the consultation is complete.")
            room.delete()

            return Response({
                'success': True,
                'room': serialized_data,
                'message': 'Consultation ended successfully and room removed'
            })
...
class MedicationReminderViewSet(viewsets.ModelViewSet):
    ...
    def send_test_email(self, request, pk=None):
        """
        Send an immediate test email for this reminder to the logged-in patient.
        Works WITHOUT Celery — calls send_mail directly.
        """
        from django.core.mail import send_mail
        ...
        if not patient_email:
            return Response(
                {'success': False, 'error': 'No email address on your account. Please update your profile.'},
                status=status.HTTP_400_BAD_REQUEST
            )

        time_slot = reminder.time_slots[0] if reminder.time_slots else 'your scheduled time'
        ...
        subject = f'💊 Medicine Reminder: {reminder.medication_name} — Rural HealthCare'
        message = f"""
Hello {reminder.patient.get_full_name() or reminder.patient.username},

⏰ This is your medicine reminder from Rural HealthCare.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
💊  Medicine    : {reminder.medication_name}
📏  Dosage      : {reminder.dosage}
🔁  Frequency   : {reminder.get_frequency_display()}
⏰  Scheduled   : {', '.join(reminder.time_slots) if reminder.time_slots else 'As needed'}
...
💡 Tips:
  ...
  • Never skip a dose — consistency is key to recovery
...
"""
        try:
            ...
            logger.info(f'[send_test_email] ✅ Sent test reminder to {patient_email} for {reminder.medication_name}')
            return Response({
                'success': True,
                'message': f'Test email sent to {patient_email}',
                'email': patient_email,
            })
        except Exception as e:
            logger.error(f'[send_test_email] ❌ Failed to send email: {str(e)}')
            return Response(
                {'success': False, 'error': f'Email send failed: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )

    @action(detail=False, methods=['post'], url_path='send-ai-insights-email')
    def send_ai_insights_email(self, request):
        from django.core.mail import send_mail
        from django.conf import settings as django_settings
        ...
        if not patient_email:
            return Response(
                {'success': False, 'error': 'No email address on your account. Please update your profile first.'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not reminders.exists():
            return Response(
                {'success': False, 'error': 'No active medicine reminders found to analyse.'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        for reminder in reminders:
            r_total  = reminder.logs.count()
            r_taken  = reminder.logs.filter(status='taken').count()
            r_missed = r_total - r_taken
            r_rate   = round((r_taken / r_total * 100), 1) if r_total > 0 else 100.0

            total_logs  += r_total
            taken_logs  += r_taken
            missed_logs += r_missed

            times_str = ', '.join(reminder.time_slots) if reminder.time_slots else 'Not set'
            reminder_rows.append(
                f"  💊 {reminder.medication_name} ({reminder.dosage})\n"
                f"     ⏰ Times: {times_str} | 🔁 {reminder.get_frequency_display()}\n"
                f"     📊 Adherence: {r_taken}/{r_total} doses taken ({r_rate}%)\n"
            )

        overall_rate = round((taken_logs / total_logs * 100), 1) if total_logs > 0 else 100.0

        # ── Risk level & recommendations (mirrors frontend logic) ──
        if overall_rate >= 80:
            risk_level = 'LOW ✅'
            risk_emoji = '🟢'
            recommendations = [
                ...
                'Share your progress with your doctor at the next visit.',
            ]
            insights = 'You are doing great! Consistent medication adherence is key to long-term health.'
        elif overall_rate >= 60:
            risk_level = 'MEDIUM ⚠️'
            risk_emoji = '🟡'
            recommendations = [
                'Try to take medications at the same time each day.',
                'Use the "Mark as Taken" button immediately after taking your medication.',
                'Link medications to daily habits like meals or brushing your teeth.',
                'Enable email reminders for medicines you frequently miss.',
            ]
            insights = 'Your adherence can be improved. Small consistent habits make a big difference.'
        else:
            risk_level = 'HIGH 🔴'
            risk_emoji = '🔴'
            recommendations = [
                ...
                'Ask a family member or caregiver to help remind you.',
                'Enable email reminders for all your medicines.',
                'Consider a weekly pill organiser for better tracking.',
            ]
            insights = (
                'Your adherence is below the recommended threshold. '
                'Missing doses can reduce the effectiveness of your treatment. '
        ...
        subject = f'🤖 AI Medication Insights Report — {patient_name} | Rural HealthCare'
        message = f"""
Hello {patient_name},

Here is your personalised AI Medication Adherence Insights Report generated on {now_str}.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📊  OVERALL ADHERENCE SUMMARY
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {risk_emoji}  Predicted Adherence Rate : {overall_rate}%
  ⚡  Risk Level              : {risk_level}
  💊  Active Medicines         : {reminders.count()}
  ✅  Total Doses Taken        : {taken_logs}
  ❌  Total Doses Missed       : {missed_logs}
  📋  Total Doses Logged       : {total_logs}
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
This report is generated automatically based on your logged medication data.
It is NOT a substitute for professional medical advice.
Always consult your doctor or pharmacist before making any changes to your medication routine.

To view and manage your reminders, visit: http://localhost:3000/medicines

Stay healthy! 🌟
Rural HealthCare AI Team
"""
        ...
        try:
            ...
            logger.info(f'[send_ai_insights_email] ✅ Insights emailed to {patient_email} for {patient_name}')
            return Response({
                'success': True,
                'message': f'AI Insights report sent to {patient_email}',
                'email': patient_email,
                'adherence_rate': overall_rate,
                'risk_level': risk_level,
            })
        except Exception as e:
            logger.error(f'[send_ai_insights_email] ❌ Failed: {str(e)}')
            return Response(
                {'success': False, 'error': f'Email send failed: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def run_email_reminders_now(request):
    """
    Manually trigger the email reminder scheduler right now.
    This is the no-Celery alternative — call this endpoint every minute
    from a simple cron job or the Django dev server startup.
    """
    from .tasks import schedule_medicine_reminders
    try:
        result = schedule_medicine_reminders()
        return Response({'success': True, 'result': result})
    except Exception as e:
        logger.error(f'[run_email_reminders_now] Error: {str(e)}')
...
class ConversationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user and self.request.user.is_authenticated:
            queryset = queryset.filter(Q(user=self.request.user) | Q(user_id_anonymous=str(self.request.user.id)))
            logger.info(f"[ConversationViewSet] Filtering by authenticated user: {self.request.user.username}")
        elif user_id:
            queryset = queryset.filter(Q(user_id_anonymous=user_id) | Q(user__id=user_id))
            logger.info(f"[ConversationViewSet] Filtering by anonymous/user ID: {user_id}")
        else:
    ...
    def retrieve(self, request, pk=None):
        try:
            ...
            if request.user and request.user.is_authenticated:
                if (conversation.user and conversation.user.id == request.user.id) or (conversation.user_id_anonymous == str(request.user.id)):
            ...
            if not user_owns_conversation:
                user_id = request.query_params.get('user_id')
                logger.info(f"[ConversationViewSet] User ID check, user_id: {user_id}")
                if user_id:
                    if conversation.user_id_anonymous == user_id or (conversation.user and str(conversation.user.id) == user_id):
                        user_owns_conversation = True
                        logger.info(f"[ConversationViewSet] User owns conversation via ID check")
    ...
    def destroy(self, request, pk=None):
        try:
            ...
            if not user_owns_conversation:
                ...
                logger.info(f"[ConversationViewSet] User ID check for delete, user_id: {user_id}")
                if user_id:
                    if conversation.user_id_anonymous == user_id or (conversation.user and str(conversation.user.id) == user_id):
                        user_owns_conversation = True
                        logger.info(f"[ConversationViewSet] User owns conversation via ID check")
...
def voice_to_text(request):
    ...
    try:
        ...
        user_id_anon = request.data.get('user_id', '')

        if selected_language and selected_language != 'English':
            language = selected_language

        logger.info(f"[voice_to_text] language={language}, user={user_id_anon}")

        if not audio_file:
            return Response({'success': False, 'error': 'No audio file received'}, status=400)

        groq_api_key = os.getenv('GROQ_API_KEY')
        if not groq_api_key:
            return Response({'success': False, 'error': 'GROQ_API_KEY not configured'}, status=500)

        try:
            from groq import Groq
        except ImportError:
            return Response({'success': False, 'error': 'groq package not installed. Run: pip install groq'}, status=500)

        client = Groq(api_key=groq_api_key)

        audio_bytes = audio_file.read()
        file_size = len(audio_bytes)
        logger.info(f"[voice_to_text] Audio received: name={audio_file.name}, size={file_size}B, type={audio_file.content_type}")
        ...
        target_lang_code = whisper_lang_map.get(language, 'en')

        lang_prompts = {
            'Malayalam': 'Medical question in Malayalam.',
            'Hindi': 'Medical question in Hindi.',
            'Tamil': 'Medical question in Tamil.',
            'Telugu': 'Medical question in Telugu.',
            'Kannada': 'Medical question in Kannada.',
            'English': 'Medical question in English.',
        }
        whisper_prompt = lang_prompts.get(language, 'Medical question.')
        ...
        try:
            logger.info(f"[voice_to_text] Sending to Groq Whisper: lang={target_lang_code}, size={file_size}B")
            ...
            logger.info(f"[voice_to_text] Raw transcription: '{text}'")

            noise_phrases = {
                '', '.', '..', '...', '….', '…',
                'this is a medical question',
                'this is a medical question in english',
                'this is a medical question in hindi',
                'this is a medical question in malayalam',
                'this is a medical question in tamil',
                'this is a medical question in telugu',
                'this is a medical question in kannada',
                'medical question in malayalam.',
                'medical question in hindi.',
                'medical question in tamil.',
                'medical question in telugu.',
                'medical question in kannada.',
                'medical question in english.',
                'medical question.',
                'thank you.',
                'thanks.',
                'you',
            ...
            clean_check = text.lower().strip().rstrip('.').strip()

            if not text or len(text.strip()) < 2 or clean_check in noise_phrases:
                error_msg = f'Microphone captured silence or background noise. (Debug: "{raw_text}") Please check your mic and speak clearly.'
                logger.warning(f"[voice_to_text] Filtered noise/empty: '{text}'")
                text = ''
                success = False
            else:
                success = True
                ...
                if native_detected:
                    final_lang = native_detected
                    logger.info(f"[voice_to_text] Native script detected: {final_lang}")
                else:
                    final_lang = language
                    logger.info(f"[voice_to_text] Using selected language: {final_lang}")

        except Exception as whisper_err:
            error_msg = f"Transcription failed: {str(whisper_err)}"
            logger.error(f"[voice_to_text] Whisper error: {whisper_err}", exc_info=True)
        finally:
            try:
                os.remove(tmp_path)
            except Exception:
        ...
        try:
            ...
            rec = VoiceRecording(
                user=user_obj,
                user_id_anonymous=user_id_anon if not user_obj else '',
            ...
            rec.audio_file.save(f"voice_{uuid.uuid4()}.{ext}", ContentFile(audio_bytes), save=False)
            rec.save()
        except Exception as db_err:
            logger.warning(f"[voice_to_text] Could not save VoiceRecording: {db_err}")

        if success:
            logger.info(f"[voice_to_text] Success: '{text}' | lang={final_lang}")
            return Response({
                'success': True,
                'text': text,
                'language': final_lang,
            ...
            return Response({
                'success': False,
                'error': error_msg or 'Could not understand speech. Please try again.',
            }, status=200)

    except Exception as e:
        logger.error(f"[voice_to_text] Critical error: {e}", exc_info=True)
        return Response({'success': False, 'error': 'Server error. Please try again.'}, status=500)


@api_view(['POST'])
@permission_classes([AllowAny])
@csrf_exempt
def process_copilot_audio(request):
    """
    Real-Time AI Symptom Co-Pilot view.
    Takes a 5-10 second audio chunk (patient voice), translates it to English using Groq Whisper,
    extracts symptoms / follow-up questions using Groq LLaMA, and broadcasts it to the room group.
    """
    import tempfile, os, json, uuid
    from channels.layers import get_channel_layer
    from asgiref.sync import async_to_sync
    from django.utils import timezone
    from .models import VideoConsultationRoom
    ...
    try:
        ...
        if not audio_file:
            return Response({'success': False, 'error': 'No audio file received'}, status=400)
        if not room_id:
            return Response({'success': False, 'error': 'No room_id received'}, status=400)

        # Look up doctor_id from room
        try:
            room = VideoConsultationRoom.objects.get(room_id=room_id)
            doctor_id = str(room.doctor.id)
        except VideoConsultationRoom.DoesNotExist:
        ...
        try:
            ...
            return Response({'success': False, 'error': 'groq package not installed.'}, status=500)

        client = Groq(api_key=groq_api_key)

        audio_bytes = audio_file.read()
        file_size = len(audio_bytes)
        logger.info(f"[process_copilot_audio] Chunk received: size={file_size}B for room={room_id}")
        ...
        try:
            # 1. Translate native speech directly to English via Whisper Large V3 on Groq (FREE)
            # This handles Hindi, Malayalam, Tamil, Telugu, Kannada, etc. and outputs clean English!
            logger.info(f"[process_copilot_audio] Sending chunk to Groq Whisper for Translation...")
            with open(tmp_path, 'rb') as f:
            ...
            raw_text = translation.text if hasattr(translation, 'text') else str(translation)
            text = (raw_text or '').strip()
            logger.info(f"[process_copilot_audio] Translated transcript: '{text}'")

            # Ignore noise/silence
            noise_checks = {'', '.', '..', '...', 'thank you.', 'thanks.', 'you.', 'you'}
            if not text or len(text) < 3 or text.lower().strip() in noise_checks:
                logger.info("[process_copilot_audio] Skipping empty or noisy transcript.")
                return Response({'success': True, 'skipped': True, 'message': 'Silence/noise'})

            # 2. Extract symptoms & suggestions using LLaMA-3.3-70b-versatile on Groq (FREE)
            prompt = f"""You are a medical AI assistant helping a doctor during a live consultation.

The patient spoke in their native language, which was translated to English:
"{text}"

Extract the following details and return ONLY a valid JSON object matching this schema:
{{
  "symptoms": ["list of symptoms mentioned"],
  "duration": "how long symptoms present (e.g. 3 days, 1 week, or null)",
  "severity": "mild/moderate/severe/unknown",
  "medications_mentioned": ["any current medicines the patient mentioned"],
  "suggested_questions": ["3 follow-up questions the doctor should ask this patient next"],
  "possible_conditions": ["2-3 potential differential diagnoses or health areas to consider"],
  "red_flags": ["any urgent emergency warning signs detected in the speech, or null"]
}}

Be concise. Do not add any conversational text before or after the JSON."""

            logger.info(f"[process_copilot_audio] Extracting symptoms using LLaMA...")
            ...
            logger.info(f"[process_copilot_audio] Extracted suggestions successfully.")

            # 3. Broadcast to the WebSocket room (doctor only)
            channel_layer = get_channel_layer()
            async_to_sync(channel_layer.group_send)(
                f'video_room_{room_id}',
                {
                    'type': 'copilot_update',
                    'suggestions': suggestions,
                    'transcript': text,
                    'detected_language': 'Auto (Whisper-Trans)',
