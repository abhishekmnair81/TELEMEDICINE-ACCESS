...
class MedicationReminder(models.Model):
    ...
    email_reminders_enabled = models.BooleanField(
        default=False,
        help_text='Send email reminders to patient at each scheduled time slot'
    )
...
class VoiceRecording(models.Model):
    """Store voice recordings sent to the medical chatbot"""
    ...
    user_id_anonymous = models.CharField(max_length=100, blank=True, default='')

    # Audio file (optional — stored only if SAVE_VOICE_AUDIO=True in settings)
    audio_file = models.FileField(upload_to='voice_recordings/%Y/%m/', null=True, blank=True)
    audio_size_bytes = models.IntegerField(default=0)
    audio_format = models.CharField(max_length=10, default='webm')  # webm, ogg, mp4
    ...
    detected_language = models.CharField(max_length=20, choices=LANGUAGE_CHOICES, default='English')
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['transcription_success']),
        ]
